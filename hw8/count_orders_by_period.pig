...
morder = FOREACH recent GENERATE SUBSTRING(order_dtm,0,7) AS year_month;

-- TODO (B): Count the number of orders in each month
group_morder = GROUP morder BY year_month;
order_count = FOREACH group_morder GENERATE group,COUNT(morder.year_month);
